...
///
/// Use this class to create a node that represents a specific location in augmented reality. The LocationNode class is a subclass of SCNNode and provides additional functionality specific to representing locations.
class LocationNode: SCNNode {
  ...
  private func setup() {
    ...
    view.configure(
      name: location.name, category: category.name, categoryImageURL: category.iconURL,
      distance: 0.0)
    ...
    let boxGeometry = SCNBox(
      width: self.width, height: self.height, length: 0.61, chamferRadius: 0.5)
  ...
  public func updateTransform(
    _ transform: matrix_float4x4, withScalingFromPOV pointOfView: SCNNode?
  ) {
