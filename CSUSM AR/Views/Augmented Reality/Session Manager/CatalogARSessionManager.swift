...
/// This class handles the setup and configuration of the AR session used in the catalog view. It provides methods for starting and stopping the AR session, as well as handling AR session events and updates.
///
/// - Note: This class is designed to be used specifically in the catalog view of the CSUSM AR app.
public class CatalogARSessionManager: NSObject {
  ...
  fileprivate func initializeGARSession() {
    do {
      self.garSession = try GARSession(
        apiKey: Secrets.GoogleAPI.SandboxGoogleAPIKey!, bundleIdentifier: nil)
  ...
  ///
  /// This method checks the current frame pair of the AR session and updates the localization state accordingly.
  /// If the Earth state is not enabled, the localization state is set to `.failed`.
  /// If the tracking state is not tracking, the localization state is set to `.pretracking`.
  /// If the localization state is `.pretracking`, it is updated to `.localizing`.
  /// If the localization state is `.localizing`, it checks the geospatial transform's horizontal accuracy and orientation yaw accuracy.
  /// If both accuracies are below the low thresholds, the localization state is set to `.localized`, and anchors are created for the catalog.
  /// If the localization state has been in `.localizing` state for more than the localization failure time, the localization state is set to `.failed`.
  /// If the geospatial transform is nil, the localization state is set to `.localizing`, and the last began localizing time is updated.
  /// If the geospatial transform's horizontal accuracy or orientation yaw accuracy is above the high thresholds, the localization state is set to `.localizing`, and the last began localizing time is updated.
  ///
  @available(*, deprecated, renamed: "updateLocalizationState")
  func _updateLocalizationState() {
    let geospatialTransform = self.garSession?.currentFramePair?.garFrame.earth?
    ...
    if self.garSession?.currentFramePair?.garFrame.earth?.earthState != .enabled {
      ...
      if self.localizationState == .pretracking {
        ...
        if let geospatialTransform = geospatialTransform,
          geospatialTransform.horizontalAccuracy <= self.kHorizontalAccuracyLowThreshold
            && geospatialTransform.orientationYawAccuracy
              <= self.kOrientationYawAccuracyLowThreshold
        {
        ...
        } else if Date.now.timeIntervalSince(self.lastBeganLocalizing)
          >= self.kLocalizationFailureTime
        {
          self.localizationState = .failed
        }
      } else {
        if geospatialTransform == nil {
          ...
          geospatialTransform.horizontalAccuracy > self.kHorizontalAccuracyHighThreshold
            || geospatialTransform.orientationYawAccuracy
              > self.kOrientationYawAccuracyHighThreshold
        {
          self.localizationState = .localizing
          self.lastBeganLocalizing = now
        }
  ...
  func updateLocalizationState() {
    ...
    case .enabled:
      if earth.trackingState != .tracking {
        ...
        default:
          self.localizationState =
            (geospatialTransform == nil
              || (geospatialTransform?.horizontalAccuracy ?? 0
                > self.kHorizontalAccuracyHighThreshold)
              || (geospatialTransform?.orientationYawAccuracy ?? 0
                > self.kOrientationYawAccuracyHighThreshold))
            ? .localizing : .failed
          self.lastBeganLocalizing = now
        }
  ...
  func updateStatistics() {
    if self.localizationState == .failed {
      ...
      self.statisticsPublisher.send(
        CatalogARSessionStatistics(errorMessage: "Not Tracking Environment"))
    } else if let transform = self.garSession?.currentFramePair?.garFrame.earth?
      .cameraGeospatialTransform
    {
      self.statisticsPublisher.send(
        CatalogARSessionStatistics(
  ...
  func createAnchors(for locations: [Concept3DLocation]) {
    ...
    guard
      let transform = self.garSession?.currentFramePair?.garFrame.earth?.cameraGeospatialTransform
    else {
  ...
  /**
     ...
     */
  func resolveAnchor(
    _ anchor: GARAnchor?, forLocation location: Concept3DLocation,
    withState state: GARRooftopAnchorState
  ) {
    guard let garAnchor = anchor, garAnchor.hasValidTransform, state == .success else {
      ...
      print(
        "State → \(state.description), Valid Transform → \(String(describing: anchor?.hasValidTransform))"
      )
      return
    }
    let arAnchor = ARAnchor(transform: garAnchor.transform)
    let identifier = AnchorManager.UUIDPair(
      arIdentifier: arAnchor.identifier, garIdentifier: garAnchor.identifier)
    self.anchorManager.addAnchors(
      uuidPair: identifier,
      data: AnchorManager.AnchorData(
        arAnchor: arAnchor, garAnchor: garAnchor, location: location, category: self.category))
