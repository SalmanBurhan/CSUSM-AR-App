...
public struct ICalParser {
    ...
    private func buildEvents(components: [ICalComponent]) -> [ICalEvent] {
        ...
        return components.map { component -> ICalEvent in
            ...
            event.trumbaFields = component.buildProperty(of: Constant.Prop.trumbaCustomField)
            event.organization = component.buildProperty(of: Constant.Prop.trumbaOrganization)
            event.eventType = component.buildProperty(of: Constant.Prop.trumbaEventType)
            event.url = component.buildProperty(of: Constant.Prop.trumbaLink)
            event.imageURL = component.buildProperty(of: Constant.Prop.trumbaImage)
    ...
    private func findComponent(
        ...
        let _elements = elements.map({
            (name: $0.name.trimmingCharacters(in: .whitespacesAndNewlines), value: $0.value.trimmingCharacters(in: .whitespacesAndNewlines))
        })
