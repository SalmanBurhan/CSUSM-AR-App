...
struct PropertyBuilder {
    ...
    /// Trumba Custom Fields
    static func buildTrumbaField(propName: String, value: String) -> XTrumbaField? {
        var trumbaField:XTrumbaField? = XTrumbaField()
        propName.replacing(/\"(.*)\"/, with: { $0.1 })
             .components(separatedBy: ";")
             ...
             .forEach { k,v in
                 switch k {
                 case Constant.Prop.TrumbaCustomField.name:
                     trumbaField?.name = v
                 case Constant.Prop.TrumbaCustomField.id:
                     trumbaField?.id = v
                 case Constant.Prop.TrumbaCustomField.type:
                     trumbaField?.type = v
                 default:
                     trumbaField = nil
                 }
        ...
        if (trumbaField != nil) {
            trumbaField?.value = value
        }
        return trumbaField
    }
    
    /// Image
    static func buildImage(value: String) -> URL? {
        let pattern = /href=\"((?:http|https:)\/\/[A-Za-z0-9.]+\/.*)\"/
        if let match = value.firstMatch(of: pattern.repetitionBehavior(.reluctant)) {
    ...
    static func buildDescription(html: String) -> String? {
        let document = try? SwiftSoup.parse(html)
        guard let text = try? document?.select("p").text(trimAndNormaliseWhitespace: true)
        else { return nil }
